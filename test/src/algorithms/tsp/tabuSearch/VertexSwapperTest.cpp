...
TEST(VertexSwapperTest, GetNextNeighbour)
{
    const auto graph = getGraph6();
    const auto path = qs::algo::tsp::makeTspResult({0, 1, 2, 3, 4, 5, 0}, graph.first);

    auto vertexSwapper = qs::algo::tsp::ts::VertexSwapper{path, graph.first};

    auto nextNeighbour = vertexSwapper.getNextNeighbour();
    auto neighbourPath = vertexSwapper.getNeighbourByDescription(nextNeighbour.first);

    EXPECT_EQ(qs::algo::tsp::getPathLength(neighbourPath, graph.first), nextNeighbour.second);

    nextNeighbour = vertexSwapper.getNextNeighbour();
    neighbourPath = vertexSwapper.getNeighbourByDescription(nextNeighbour.first);
