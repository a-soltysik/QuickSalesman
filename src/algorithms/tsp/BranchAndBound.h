...
{
    { c.seize_top() } -> std::convertible_to<typename T::value_type>;
...
private:
    ...
    [[nodiscard]]
    auto makePath() -> bnb::NodeData::Path;
...
template<bnb::Container T>
auto BranchAndBound<T>::makeNodeForEveryNeighbour(T& container, const bnb::NodeData& node,
                                                  const bnb::NodeData& optimum) -> void
{
    for (auto i = tsplib::Graph::Vertex {}; i < graphOrder; i++)
    {
        if (node.reducedMatrix[node.path[node.level]][i] < optimum.cost)
        {
...
template<bnb::Container T>
auto BranchAndBound<T>::makeMatrixGraph() -> bnb::NodeData::MatrixGraph
{
...
template<bnb::Container T>
auto BranchAndBound<T>::copyPath(const bnb::NodeData::Path& path) -> bnb::NodeData::Path
{
    auto result = makePath();
    std::copy(path.get(), path.get() + graphOrder + 1, result.get());
    return result;
}

template<bnb::Container T>
auto BranchAndBound<T>::copyMatrix(const bnb::NodeData::MatrixGraph& matrix) -> bnb::NodeData::MatrixGraph
{
...
template<bnb::Container T>
auto BranchAndBound<T>::makePath() -> bnb::NodeData::Path
{
    return bnb::NodeData::Path(new tsplib::Graph::Vertex[graphOrder + 1]);
}
